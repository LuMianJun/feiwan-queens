...
export function openLessons({onClose=()=>{}}={}){
  ...
  dialog.innerHTML='<header><span id="lesson-count"></span><button id="lesson-exit" type="button">跳过</button></header><h2 id="lesson-title"></h2><p id="lesson-copy"></p><div id="lesson-board" role="group" aria-label="教学棋盘"></div><p id="lesson-message" role="status" aria-live="polite"></p><footer><button id="lesson-prev" type="button">上一课</button><button id="lesson-hint" type="button">给点提示</button><button id="lesson-next" type="button" disabled>下一课</button></footer>';
  ...
  function act(i,action,paint=false){
    ...
    const valid=round.act(i,action);
    $('#lesson-message').textContent=valid?(round.complete?'推理正确！':'正确。'):'再看看高亮的行、列或颜色。';
    render();
  }
  const taps=new TapInput({immediate:true,single:i=>{if(round.lesson.action==='cross')act(i,'cross');else $('#lesson-message').textContent='双击确认肥丸的位置。';},double:i=>{if(round.lesson.action==='find')act(i,'find');}});
  function render(){
    buttons.forEach((b,i)=>{
  ...
  function show(){
    ...
    });render();$('#lesson-title').tabIndex=-1;$('#lesson-title').focus({preventScroll:true});dialog.scrollTop=0;
  }
  ...
  $('#lesson-hint').onclick=()=>{const i=round.lesson.targets.find(i=>!round.done.has(i));if(i===undefined)return;buttons.forEach(b=>b.classList.remove('lesson-hint'));buttons[i].classList.add('lesson-hint');$('#lesson-message').textContent=`试着${round.lesson.action==='find'?'双击确认':'排除'}第 ${Math.floor(i/round.lesson.size)+1} 行、第 ${i%round.lesson.size+1} 列。`;};
