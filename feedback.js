export class Feedback {
  ...
  unlock(){
    ...
    try{if(!this.context||this.context.state==='closed'){this.context=this.createContext();if(!this.context)return Promise.resolve(false);this.master=this.context.createGain();this.master.gain.value=.12;this.master.connect(this.context.destination);}
      const c=this.context;return Promise.resolve(c.state==='running'?null:c.resume()).then(()=>c.state==='running').catch(()=>false);
    }catch{return Promise.resolve(false);}
  }
  setSound(value){this.sound=value;if(this.master)this.master.gain.value=value?.12:0;if(!value)this.epoch++;}
  ...
  play(kind){
    ...
    if(this.vibration&&this.vibrate)try{this.vibrate(kind==='wrong'?[45,35,45]:kind==='win'?[25,35,25]:kind==='correct'?25:8);}catch{}
    ...
    void this.unlock().then(ready=>{if(!ready||!this.sound||epoch!==this.epoch||Date.now()-started>250)return;
      try{const c=this.context;this.master.gain.value=.12;const notes=kind==='wrong'?[190,140]:kind==='win'?[523,659,784]:kind==='correct'?[660,880]:[kind==='erase'?350:470];
