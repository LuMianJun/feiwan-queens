...
export class Feedback {
  ...
  play(kind){
    ...
    if(kind!=='record'&&this.vibration&&this.vibrate)try{this.vibrate(kind==='wrong'?[80,45,80]:kind==='win'?[50,45,70]:kind==='correct'?55:kind==='drag'?18:25);}catch{}
    ...
    void this.unlock().then(ready=>{if(!ready||!this.sound||epoch!==this.epoch||Date.now()-started>250)return;
      try{const c=this.context;this.master.gain.value=MASTER_VOLUME;const record=kind==='record',notes=record?[659,880]:kind==='wrong'?[190,140]:kind==='win'?[523,659,784]:kind==='correct'?[660,880]:[kind==='erase'?350:470];
